...
const conditions = {
  'pde-field': pair('Края соединены. Штриховая кривая — u(x,0); для волны uₜ(x,0)=0.', 'Periodic ends. Dashed curve: u(x,0); the wave starts with uₜ(x,0)=0.'),
  ...
  'advection-diffusion': pair('Края соединены. Pe=2V/D: при большом Pe преобладает перенос, при малом — диффузия.', 'Periodic edges. Pe=2V/D: large Pe means transport dominates; small Pe means diffusion dominates.'),
  'vector-calculus': pair('Цвет — выбранная производная, стрелки — скорость. Нулевая дивергенция не означает отсутствие движения.', 'Color: selected derivative; arrows: velocity. Zero divergence does not mean no motion.'),
  ...
  'shallow-water': pair('Плоское дно; глубина h>0. Внешние стенки отражают волны. Цвет — выбранное поле, стрелки — скорость воды.', 'Flat bed; wet depth h>0. Outer walls reflect waves. Color: selected field; arrows: water velocity.'),
  ...
  sources: pair('Края соединены. u — знакопеременное отклонение температуры; S<0 означает охлаждение.', 'Periodic edges. u is a signed temperature deviation; S<0 means cooling.'),
...
export function lessonPresentation(model, entry, language) {
  ...
  if (model.id === 'riemann') {
    ...
    else detail += language === 'ru' ? 'Жёлтая линия — контакт; красные — ударные волны; голубые — границы разрежения.' : 'Yellow: contact; red: shocks; blue: rarefaction edges.';
  }
