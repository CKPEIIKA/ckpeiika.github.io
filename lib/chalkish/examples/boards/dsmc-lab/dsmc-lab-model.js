...
function validateParameters(base, patch) {
  ...
  if (parameters.collisionLineProbability < 0 || parameters.collisionLineProbability > 1) {
    throw unchangedError(
      RangeError,
      'collisionLineProbability must be between zero and one.',
    );
  }
...
export class DsmcLabModel {
  ...
  step(dt) {
    ...
    const result = this._core.collisionStep({
      ...
      collisionLineProbability: this._parameters.highlightEvents
        ? this._parameters.collisionLineProbability
        : 0,
  ...
  _computeDiagnostics(moments = this._moments()) {
    ...
    return Object.freeze({
      ...
      collisionLineProbability: this._parameters.collisionLineProbability,
      healthy,
