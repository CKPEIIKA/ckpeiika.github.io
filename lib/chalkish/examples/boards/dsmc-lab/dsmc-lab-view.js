...
function particlePalettes(styleName) {
  const clean = styleName === 'clean';
  return {
    primary: [
      {
        fill: '#eee9d7',
        stroke: clean ? '#ffffff' : null,
        radius: clean ? 0.9 : 1.15,
        opacity: clean ? 0.88 : 0.67,
        width: 0.45,
      },
    ...
    secondary: [
      { fill: '#efd47d00', stroke: null, radius: 1, opacity: 0 },
      {
        fill: clean ? '#efd47d' : '#e9cf73',
        stroke: clean ? '#fff2b8' : '#f6e9ad',
...
export function bindDsmcLabView(
  ...
  const initialParticles = particlePalettes(styleName);
  ...
  function updateView(snapshot, diagnostics = null) {
    ...
    heading.setText(title(next.metadata.caseId));
    method.setText(formula(next.metadata.caseId));
  ...
  function setStyle(name) {
    ...
    const particleStyles = particlePalettes(name);
