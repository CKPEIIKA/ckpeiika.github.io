...
export const CHALK_FONT_STACK = '"Neucha", "Shantell Sans", sans-serif';
export const CHALK_CLEAN_FONT_STACK = '"Shantell Sans", sans-serif';
...
/**
 * Deposit the current Canvas path as a restrained technical chalk stroke.
 * Geometry stays with the caller; Chalkish owns the shared material treatment.
 */
export function strokeChalkPath2D(context) {
  if (!context || typeof context.stroke !== 'function' || typeof context.save !== 'function') {
  ...
  const offset = context.lineDashOffset || 0;
  const scale = Math.max(0.7, width);
  const passes = width < 0.65
    ? [{ alpha: 0.82, width: 1, x: 0, y: 0, dash: null, offset: 0 }]
    : [
      { alpha: 0.72, width: 1, x: 0, y: 0, dash: null, offset: 0 },
      { alpha: 0.30, width: 0.72, x: 0.34, y: -0.24, dash: [5.4, 0.8, 2.1, 1.25], offset: 0.7 },
      { alpha: 0.20, width: 0.54, x: -0.28, y: 0.22, dash: [1.2, 1.8, 4.6, 0.9], offset: 2.3 },
    ];

  for (const pass of passes) {
    context.save();
    context.globalAlpha = alpha * pass.alpha;
    context.lineWidth = Math.max(0.45, width * pass.width);
    context.lineCap = 'round';
    context.lineJoin = 'round';
    context.translate(pass.x * scale, pass.y * scale);
    if (typeof context.setLineDash === 'function') {
      context.setLineDash(dash.length ? dash : (pass.dash || []));
      context.lineDashOffset = offset + pass.offset * scale;
    }
    context.stroke();
    context.restore();
  }
