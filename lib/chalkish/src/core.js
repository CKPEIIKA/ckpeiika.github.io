...
export class Camera2D {
  ...
  // Fit a world-space rectangle without cropping at narrow aspect ratios.
  fitBounds({ minX, maxX, minY, maxY }, width, height, padding = 0.08) {
    [minX, maxX, minY, maxY].forEach(value => assertFinite(value, 'bound'));
    assertPositive(maxX - minX, 'bounds width');
    assertPositive(maxY - minY, 'bounds height');
    assertPositive(width, 'viewport width');
    assertPositive(height, 'viewport height');
    ...
    return this.setHeight(Math.max(w * s + h * c, (w * c + h * s) * height / width) * (1 + 2 * padding));
  }
