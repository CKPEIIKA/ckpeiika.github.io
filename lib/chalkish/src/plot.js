...
export class SampledPlot extends Group {
  constructor({ xRange = [-1, 1], yRange = [-1, 1], width = 10, height = 3, title = '', ...options } = {}) {
    ...
    this.axes = new Group();
    this.records = [];
    this.title = new TextLabel(title, { x: -width / 2, y: height / 2 + 0.3, align: 'left', font: '15px "Neucha", "Shantell Sans", sans-serif', style: labelStyle });
    ...
    for (const fraction of [0, 0.5, 1]) {
      ...
      this.axes.add(
        ...
        new TextLabel(this.format(xRange[0] + fraction * (xRange[1] - xRange[0])), {
          x, y: -this.height / 2 - 0.23, font: '12px "Neucha", "Shantell Sans", sans-serif', style: labelStyle,
        }),
        new TextLabel(this.format(yRange[0] + fraction * (yRange[1] - yRange[0])), {
          x: -this.width / 2 - 0.15, y, align: 'right', font: '12px "Neucha", "Shantell Sans", sans-serif', style: labelStyle,
        }),
      );
    }
    return this;
