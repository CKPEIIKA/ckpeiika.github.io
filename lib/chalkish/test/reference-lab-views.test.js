...
import { createDgFvLabController } from '../examples/boards/shared/dg-fv-lab-controller.js';
import { createDsmcLabController } from '../examples/boards/shared/dsmc-lab-controller.js';
import {
  CartesianGrid,
  SegmentLayer,
...
test('DG/FV teaching overlays remain packed and follow the selected case', () => {
  const controller = createDgFvLabController({
    model: {
      caseId: 'euler-cylinder',
      columns: 32,
  ...
  const { freestream, wedgeReference, meanCurve } = controller.view.layers;
  assert.ok(freestream instanceof SegmentLayer);
  assert.equal(freestream.count, 9);
  assert.ok(wedgeReference instanceof SegmentLayer);
  assert.equal(wedgeReference.count, 2);
  assert.equal(wedgeReference.visible, true);
  ...
  assert.equal(freestream.visible, false);
  assert.equal(wedgeReference.visible, false);
  assert.equal(meanCurve.visible, true);
  assert.equal(meanCurve.positionsY, controller.model.state.meanScalar);
  controller.dispose();
});

test('DSMC teaching layers reuse solver-owned event buffers and switch particle style', () => {
  const controller = createDsmcLabController({
    model: {
      particleCount: 256,
      seed: 17,
  ...
  const { collisionEvents, rotationalEvents, speciesBMarks, particles } = controller.view.layers;
  assert.equal(collisionEvents.segments, controller.model.state.collisionSegments);
  assert.equal(rotationalEvents.segments, controller.model.state.rotationalSegments);
  assert.equal(speciesBMarks.visible, true);
  assert.equal(speciesBMarks.shape, 'pixel');
  const dustyRadius = particles.palette[0].radius;
  controller.setStyle('clean');
  assert.ok(particles.palette[0].radius < dustyRadius);
  controller.dispose();
});
